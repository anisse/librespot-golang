module github.com/anisse/librespot-golang/src/examples/micro-client

require (
	github.com/anisse/librespot-golang/src/Spotify v0.0.1
	github.com/anisse/librespot-golang/src/librespot v0.0.1
	github.com/xlab/portaudio-go v0.0.0-20170905165025-132d041879db
	github.com/xlab/vorbis-go v0.0.0-20170905164921-1de75b5f6997
)

replace github.com/anisse/librespot-golang/src/Spotify => ../../Spotify

replace github.com/anisse/librespot-golang/src/librespot => ../../librespot
